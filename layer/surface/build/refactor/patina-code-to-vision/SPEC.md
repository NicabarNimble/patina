...
| CV7 | verified-false | `cargo run -q -- mother status` now reports project needs and marks `session-writer` as `missing` (not loaded/visible in daemon children list). |
| CV8 | verified-partial | Project manifest contract exists at `.patina/manifest.toml` (`schema = 1`, `[needs].children = [...]`) and `mother status` reads/report child availability, but automatic install/resolution on connect is not yet implemented. |
