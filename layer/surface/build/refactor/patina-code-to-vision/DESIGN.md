...
- Added daemon child markers for `spec-manager`, `doctor`, and `lake-manager`, so manifest resolution now reports all required children as loaded/ok in status output.
