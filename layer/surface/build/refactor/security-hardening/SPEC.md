...
| 2026-02-04 | in-progress | Two pre-existing bugs fixed: (1) Added `/secrets/cache` (GET+POST) and `/secrets/lock` (POST) endpoints to serve daemon — `session.rs` called them but they were never wired, secrets never cached. `SecretsCacheEntry` with TTL in `ServerState` via `Mutex`. (2) UDS EOF fix: `handle_connection` changed to borrow stream, accept loops call `shutdown(Write)` on concrete type after response — client `read_to_end` sees EOF promptly instead of waiting for socket close or timeout. |
