...
pub struct BeliefEntry {
    pub id: String,
    pub source: String,
    pub source_project_uid: Option<String>,
...
impl Graph {
    ...
    pub fn sync_beliefs(&self, entries: &[BeliefEntry], synced_sources: &[String]) -> Result<()> {
        ...
        for entry in entries {
            if let Err(e) = self.conn.execute(
                r#"
                INSERT INTO beliefs (id, source, source_project_uid, source_project_id,
                    source_commit_sha, source_revision,
                    kind, statement, entrenchment, status, facets,
    ...
    pub fn search_beliefs(&self, query: &str, limit: usize) -> Result<Vec<BeliefEntry>> {
        ...
        let mut stmt = self.conn.prepare(
            r#"
            SELECT b.id, b.source, b.source_project_uid, b.source_project_id,
                   b.source_commit_sha, b.source_revision,
                   b.kind, b.statement, b.entrenchment, b.status, b.facets,
        ...
        let entries = stmt
            .query_map(params![sanitized, limit as i64], |row| {
                Ok(BeliefEntry {
                    id: row.get(0)?,
                    source: row.get(1)?,
                    source_project_uid: row.get(2)?,
                    source_project_id: row.get(3)?,
                    source_commit_sha: row.get(4)?,
                    source_revision: row.get(5)?,
                    kind: row.get(6)?,
                    statement: row.get(7)?,
                    entrenchment: row.get(8)?,
                    status: BeliefStatus::from_str_or_default(&row.get::<_, String>(9)?),
                    facets: row.get::<_, Option<String>>(10)?.unwrap_or_default(),
                    cited_by_beliefs: row.get(11)?,
                    cited_by_sessions: row.get(12)?,
                    applied_in: row.get(13)?,
                    evidence_count: row.get(14)?,
                    evidence_verified: row.get(15)?,
                    health_score: row.get(16)?,
                    contested_by: row.get::<_, Option<String>>(17)?.unwrap_or_default(),
                    imported: row.get::<_, i32>(18).unwrap_or(0) != 0,
                    grounding_score: row.get::<_, Option<f64>>(19)?.unwrap_or(0.0),
                    grounding_code_count: row.get::<_, Option<i32>>(20)?.unwrap_or(0),
                    grounding_commit_count: row.get::<_, Option<i32>>(21)?.unwrap_or(0),
                    grounding_session_count: row.get::<_, Option<i32>>(22)?.unwrap_or(0),
                    grounding_forge_count: row.get::<_, Option<i32>>(23)?.unwrap_or(0),
                    verification_total: row.get::<_, Option<i32>>(24)?.unwrap_or(0),
                    verification_passed: row.get::<_, Option<i32>>(25)?.unwrap_or(0),
                    verification_failed: row.get::<_, Option<i32>>(26)?.unwrap_or(0),
                    verification_errored: row.get::<_, Option<i32>>(27)?.unwrap_or(0),
                    last_activity: row.get::<_, Option<String>>(28)?,
                })
            })?
            .collect::<std::result::Result<Vec<_>, _>>()?;
