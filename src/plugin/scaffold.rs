...
/// Apply name substitutions to a template string.
...
pub fn scaffold(parent: &Path, name: &str, world: &PluginWorld) -> Result<std::path::PathBuf> {
    validate_name(name)?;
    ...
    std::fs::write(project_dir.join("plugin.toml"), substitute(plugin_tmpl, name))?;
