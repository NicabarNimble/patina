...
/// Check if we should use the refactored indexer (proper black-box wrapper)
pub fn use_refactored_indexer() -> bool {
    env::var("PATINA_USE_REFACTORED_INDEXER")
...
/// Check if we should use the refactored navigate command
pub fn use_refactored_navigate() -> bool {
    env::var("PATINA_USE_REFACTORED_NAVIGATE")
        .map(|v| v == "1" || v.to_lowercase() == "true")
        .unwrap_or(false)
}
...
pub fn print_refactor_config() {
    ...
    eprintln!("  Indexer: {}", if use_refactored_indexer() { "refactored" } else { "original" });
    ...
    eprintln!("  Navigate: {}", if use_refactored_navigate() { "refactored" } else { "original" });
}