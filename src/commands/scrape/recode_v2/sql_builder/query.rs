...
impl DeleteBuilder {
    ...
    pub fn where_eq<V: Into<SqlValue>>(mut self, column: &str, value: V) -> Self {
        ...
        self.where_clauses
            .push(format!("{} = {}", column, sql_value.to_sql()));
        self
    }
    ...
    pub fn where_in<V: Into<SqlValue>>(mut self, column: &str, values: Vec<V>) -> Self {
        if values.is_empty() {
            ...
            let sql_values: Vec<String> = values.into_iter().map(|v| v.into().to_sql()).collect();
            self.where_clauses
                .push(format!("{} IN ({})", column, sql_values.join(", ")));
        }
        self
    }
...
mod tests {
    ...
    fn test_delete_with_in() {
        ...
        assert_eq!(
            sql,
            "DELETE FROM index_state WHERE path IN ('src/a.rs', 'src/b.rs')"
        );
    }
    ...
    fn test_index_state_query() {
        ...
        assert_eq!(
            sql,
            "SELECT path || '|' || CAST(mtime AS VARCHAR) FROM index_state"
        );
    }
}
