...
pub fn create_project_design_wizard(default_name: &str, env: &Environment) -> Result<String> {
    ...
    if env.tools.get("go").is_some_and(|info| info.available) && env.tools.get("dagger").is_some_and(|info| info.available) {
...
"#,
        ...
        project_type,
        primary_language,
        if primary_language == "rust" { "min_rust_version = \"1.75\"\n" } else { "" },
        required_tools.join(", "),
