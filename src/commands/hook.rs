...
pub fn on_stop() -> Result<()> {
    ...
    let data: Value = serde_json::from_str(&input).unwrap_or_else(|_| serde_json::json!({}));
    ...
    if Path::new("layer/").exists() {
        ...
        if !status.stdout.is_empty() {
            Command::new("git").args(["add", "layer/"]).output()?;
...
fn get_comodified_files(file: &str) -> Result<Vec<String>> {
    ...
    Ok(files.into_iter().take(5).map(|(file, _)| file).collect())
}
