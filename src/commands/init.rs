...
fn validate_environment(env: &patina::Environment, design: &Value) -> Result<Option<Vec<String>>> {
    ...
    if let Some(technical) = design.get("technical") {
            ...
            match language {
                "rust" => {
                    if !env.languages.get("rust").is_some_and(|info| info.available) {
                        warnings.push(format!("⚠️  Required language '{language}' not detected"));
                    }
                }
                _ => {
                    warnings.push(format!(
                        "💡 Note: Patina is designed for Rust development (specified: {language})"
                    ));
                }
            }
        }
        ...
        if let Some("postgres" | "postgresql") = technical.get("database").and_then(|v| v.as_str())
        {
            if !env.tools.get("psql").is_some_and(|info| info.available) {
                warnings.push(
                    "💡 PostgreSQL client not found - you may need it for database operations"
                        .to_string(),
                );
            }
        }
