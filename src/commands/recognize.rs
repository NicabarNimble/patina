...
fn analyze_patterns(files: &[SurvivingFile]) -> Result<Vec<RecognizedPattern>> {
    ...
    let mut public_private = RecognizedPattern {
        ...
        description: "Module exposes public functions that delegate to private implementation"
            .to_string(),
        ..Default::default()
    };
    ...
    for file in files {
        ...
        let content =
            fs::read_to_string(&file.path).context(format!("Failed to read {}", file.path))?;
            ...
            && !content.contains("pub struct")
        {
            ...
            public_private
                .characteristics
                .push("Clear module boundaries".to_string());
        }
        ...
        if content.contains(".context(") || content.contains(".with_context(") {
            ...
            let context_count =
                content.matches(".context(").count() + content.matches(".with_context(").count();
            if context_count > 5 {
                error_context
                    .characteristics
                    .push(format!("{context_count} error contexts"));
            }
        }
        ...
        if content.contains("impl") && content.contains("Builder") && content.contains("build(") {
            ...
            builder
                .characteristics
                .push("Fluent interface for construction".to_string());
        }

        // Detect Type-State Pattern
        if content.contains("PhantomData")
            || (content.contains("enum") && content.contains("impl") && content.contains("From"))
        {
            type_state.found_in.push(file.path.clone());
            type_state
                .characteristics
                .push("Compile-time state validation".to_string());
        }
    }
        ...
        for j in 0..patterns.len() {
            if i != j {
                ...
                if intersection > 0 {
                    ...
                    patterns[i]
                        .co_patterns
                        .insert(pattern_names[j].clone(), co_occurrence);
                }
            }
        }
    }
...
fn display_patterns(patterns: &[RecognizedPattern]) -> Result<()> {
    ...
    for (idx, pattern) in patterns.iter().enumerate() {
        ...
        println!(
            "├─ {}: {} files",
            "Found in".bright_black(),
            pattern.found_in.len()
        );
        println!(
            "├─ {}: {:.1}%",
            "Prevalence".bright_black(),
    ...
    println!(
        "\n{}",
        "💡 Tip: These patterns appear in code that hasn't been modified for 6+ months"
            .bright_black()
    );
