...
pub fn execute(args: OrganizeArgs) -> Result<()> {
    println!(
        "🔍 Analyzing patterns using Git history ({} days)...\n",
        args.days
    );
...
fn analyze_with_git(path: &Path, days: u32) -> Result<GitMetrics> {
    ...
    let last_change = git_command(&["log", "-1", "--format=%at", "--", &path_str])?;

    let days_since_last_change = if !last_change.trim().is_empty() {
        let timestamp = last_change.trim().parse::<i64>().unwrap_or(0);
        let last = DateTime::from_timestamp(timestamp, 0).unwrap_or_else(Utc::now);
    ...
    let pattern_name = path.file_stem().and_then(|s| s.to_str()).unwrap_or("");
...
fn find_co_modified_files(path: &Path, days: u32) -> Result<Vec<String>> {
    ...
    for commit in commits.lines() {
        let files = git_command(&["diff-tree", "--no-commit-id", "--name-only", "-r", commit])?;
    ...
    if layer == "surface" {
        ...
        if change_count > 20 && survival_rate < 70.0 {
            return (
                ...
                format!(
                    "High churn ({change_count} changes, {survival_rate:.0}% survival) - needs splitting"
                ),
            );
        }
...
fn display_git_analysis(metrics: &[GitMetrics], verbose: bool) {
    ...
    for metric in metrics {
        ...
        by_action.entry(key).or_default().push(metric);
    }
    ...
    for metric in metrics.iter().take(5) {
        if metric.change_count > 0 {
            let name = metric
                ...
                .unwrap_or("unknown");
            println!(
                "  {} ({} layer): {} changes, {} days ago, {:.0}% survival",
                name,
                metric.layer,
                metric.change_count,
                metric.days_since_last_change,
...
fn apply_git_recommendations(metrics: &[GitMetrics]) -> Result<()> {
    ...
    if moved > 0 {
        println!("✓ Moved {moved} patterns based on Git activity");
    }
    if extracted > 0 {
        println!("✓ Created {extracted} insight extraction tasks");
    }
