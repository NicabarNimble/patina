...
pub fn execute(type_: String, name: String) -> Result<()> {
    ...
    let valid_types = ["core", "topic", "project", "decision", "constraint", "principle"];
    if !valid_types.contains(&type_.as_str()) {
        anyhow::bail!("Invalid pattern type '{}'. Valid types: {:?}", type_, valid_types);
    }
    ...
    println!("✓ Added {} '{}' to current session", type_, name);
    println!("  Patterns in session: {}", session.patterns.len());
    println!("  Uncommitted patterns: {}", session.uncommitted_patterns().len());
