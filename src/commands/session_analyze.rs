...
fn analyze_session(session_id: &str, args: &SessionAnalyzeArgs) -> Result<SessionAnalysis> {
    let start_tag = format!("session-{}-start", session_id);
    let end_tag = format!("session-{}-end", session_id);
...
fn get_session_goal(session_id: &str) -> Result<String> {
    // Try session file
    let session_file = format!("layer/sessions/{}.md", session_id);
...
fn get_file_changes(start_tag: &str) -> Result<(Vec<String>, Vec<String>)> {
    let output = Command::new("git")
        .args(["diff", "--name-status", &format!("{}..HEAD", start_tag)])
        .output()?;
...
fn get_diff_stats(start_tag: &str) -> Result<(u32, u32)> {
    let output = Command::new("git")
        .args(["diff", "--shortstat", &format!("{}..HEAD", start_tag)])
        .output()?;
...
fn analyze_patterns(created: &[String], modified: &[String]) -> Result<(Vec<String>, Vec<String>)> {
    ...
    for file in created.iter().chain(modified.iter()) {
        if file.contains("/internal/") || file.ends_with("/internal.rs") {
            if !implemented.contains(&"dependable-rust".to_string()) {
...
fn calculate_survival_rate(start_tag: &str) -> Result<f32> {
    ...
    let output = Command::new("git")
        .args(["rev-list", &format!("{}..HEAD", start_tag)])
        .output()?;
...
fn extract_insights(
    ...
    if refactor_count > 2 {
        insights.push(format!("High iteration ({} refactors) - exploring solution space", refactor_count));
    }
    
    // Insight: New subsystem
    if created.len() > 5 {
        insights.push(format!("Created new subsystem ({} new files)", created.len()));
    }
...
fn print_human_summary(analysis: &SessionAnalysis, verbose: bool) {
    ...
    println!("  Lines: +{} -{}", analysis.lines_added, analysis.lines_removed);
