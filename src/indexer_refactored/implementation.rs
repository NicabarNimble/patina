...
// All submodules are now private
mod benchmark;
mod git_detection;
mod git_state;
mod hybrid_database;
...
// Internal uses only
use git_state::{Confidence, GitEvent, GitState};
use hybrid_database::{HybridDatabase, NavigationCRDT, Pattern, WorkspaceState};
use sqlite_database::SqliteClient;
use navigation_state::{DocumentInfo, GitAwareNavigationMap, WorkspaceNavigationState};
use state_machine::GitNavigationStateMachine;
...
/// Internal implementation of the pattern indexer
...
impl IndexerImpl {
    ...
    pub fn clear(&mut self) -> Result<()> {
        ...
        *cache = GitAwareNavigationMap::new();
        ...
        // Clear database if present
        if let Some(db) = &self.db {
            match db {
                DatabaseBackend::Sqlite(sqlite_db) => {
                    // TODO: Add clear method to SqliteClient
                }
                DatabaseBackend::Hybrid(hybrid_db) => {
                    // TODO: Add clear method to HybridDatabase
                }
            }
        }
    ...
    pub fn navigate(&self, query: &str) -> super::NavigationResult {
        let cache = self.cache.lock().unwrap();
        let response = cache.navigate(query);

        // Convert internal response to public NavigationResult
        let locations = response.locations.into_iter().map(|loc| {
            super::LocationInfo {
                path: loc.path,
                line: None, // TODO: Extract line numbers if available
                confidence: match loc.confidence {
                    Confidence::Verified => 1.0,
                    Confidence::High => 0.8,
                    Confidence::Medium => 0.6,
                    Confidence::Low => 0.4,
                    Confidence::Experimental => 0.2,
                },
                preview: Some(loc.relevance),
            }
        }).collect();

        super::NavigationResult {
