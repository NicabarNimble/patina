...
fn main() -> Result<()> {
    ...
    println!("📊 Analyzed {} files, {} functions\n", file_count, all_functions.len());
    ...
    let avg_size: usize = all_functions.iter().map(|f| f.actual_line_count).sum::<usize>() 
    ...
    println!("\n🧩 Complexity Analysis:");
    let avg_complexity: usize = all_functions.iter().map(|f| f.cognitive_complexity).sum::<usize>() 
        / all_functions.len().max(1);
    ...
    println!("\n🚪 API Boundaries:");
    println!("  {} public functions that return Result", insights.api_boundaries.len());
    for boundary in insights.api_boundaries.iter().take(5) {
        println!("    - {}", boundary);
    }
    
    println!("\n⚠️  Error Handling:");
    println!("  {} functions add error context", insights.error_handlers.len());
    let no_context = all_functions.iter()
        .filter(|f| {
            f.patterns.iter().any(|p| {
                matches!(p, patina::semantic::queries::SemanticPattern::ErrorPropagation { 
                    adds_context: false, 
                    propagates: true, 
                    .. 
                })
        ...
        .count();
    println!("  {} functions propagate errors WITHOUT context", no_context);
    
    println!("\n🧪 Test Coverage:");
    println!("  {:.1}% of functions are tests", insights.test_coverage_ratio * 100.0);
    let tested_functions = all_functions.iter()
        .filter(|f| !f.called_by.is_empty())
        .count();
    println!("  {} functions are called by other functions", tested_functions);
    
    println!("\n📞 Call Patterns:");
    let functions_with_calls = all_functions.iter()
        .filter(|f| !f.calls.is_empty())
        .count();
    println!("  {} functions make calls to other functions", functions_with_calls);
    
    let most_called: Vec<_> = all_functions.iter()
    ...
    for func in &all_functions {
        for pattern in &func.patterns {
            let pattern_name = match pattern {
                patina::semantic::queries::SemanticPattern::ErrorPropagation { .. } => "ErrorPropagation",
                ...
                patina::semantic::queries::SemanticPattern::DependencyInjection { .. } => "DependencyInjection",
                patina::semantic::queries::SemanticPattern::ResourceManagement { .. } => "ResourceManagement",
            };
