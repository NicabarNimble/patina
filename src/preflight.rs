...
pub fn ensure_clean_state() {
    for proc in find_stale_processes(STALE_THRESHOLD_MINUTES) {
        if Command::new("kill").arg(proc.pid.to_string()).output().is_ok() {
            eprintln!("Cleaned up stale {} (PID {}, running {})",
                proc.cmd, proc.pid, format_elapsed(proc.mins));
        }
    }
}

struct StaleProcess { pid: u32, cmd: String, mins: u64 }

fn find_stale_processes(threshold: u64) -> Vec<StaleProcess> {
    let output = match Command::new("ps").args(["-eo", "pid,etime,command"]).output() {
    ...
    String::from_utf8_lossy(&output.stdout)
        ...
        .filter_map(|line| {
            ...
            if pid == current_pid || p.len() < 3 { return None; }
            let mins = parse_elapsed(p.get(1)?);
            if mins < threshold { return None; }
            Some(StaleProcess { pid, cmd: p[2..].join(" "), mins })
        })
        .collect()
}

fn parse_elapsed(s: &str) -> u64 {
    let (days, rest) = s.find('-').map_or((0, s), |i| (s[..i].parse().unwrap_or(0), &s[i+1..]));
    let p: Vec<u64> = rest.split(':').filter_map(|x| x.parse().ok()).collect();
    days * 1440 + match p.len() { 3 => p[0] * 60 + p[1], 2 => p[0], _ => 0 }
}
