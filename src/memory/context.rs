...
fn find_applicable_patterns(topic: &str) -> Result<Vec<String>> {
    ...
    let pattern_map = HashMap::from([
        ...
        (
            "pattern",
            vec!["dependable-rust", "pattern-selection-framework"],
        ),
        ("adapter", vec!["adapter-pattern", "versioning"]),
        (
            "build",
            vec!["container-orchestration", "dagger-integration"],
        ),
    ...
    let pattern_path = format!("layer/core/{topic}.md");
