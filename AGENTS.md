...
- WIT toy contracts live in `wit/toys/deps/` (per-interface packages). Child world composition lives in `wit/knowledge-child/`, `wit/command/`, `wit/task/`, `wit/pipeline/` (per-kind).
- SDK surface is `sdk/patina-sdk` (feature-gated toys and world exports).
