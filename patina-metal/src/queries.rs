...
impl QueryLoader {
    ...
    pub fn load_all_queries() -> HashMap<(Metal, QueryType), String> {
        ...
        // Rust queries
        queries.insert(
            (Metal::Rust, QueryType::Symbols),
            include_str!("../queries/rust/symbols.scm").to_string(),
        );
        queries.insert(
            (Metal::Rust, QueryType::Complexity),
            include_str!("../queries/rust/complexity.scm").to_string(),
        );
        queries.insert(
            (Metal::Rust, QueryType::Patterns),
            include_str!("../queries/rust/patterns.scm").to_string(),
        );

        // Go queries
        queries.insert(
            (Metal::Go, QueryType::Symbols),
            include_str!("../queries/go/symbols.scm").to_string(),
        );
        queries.insert(
            (Metal::Go, QueryType::Complexity),
            include_str!("../queries/go/complexity.scm").to_string(),
        );
        queries.insert(
            (Metal::Go, QueryType::Patterns),
            include_str!("../queries/go/patterns.scm").to_string(),
        );
