...
"#;
    ...
    let parsed = analyzer.parse(source, Metal::Rust).expect("Failed to parse");
    let matches = analyzer.run_query(&parsed, QueryType::Symbols).expect("Failed to run query");
    
    // Check we found the expected symbols
    let functions: Vec<_> = matches.iter()
    ...
    let structs: Vec<_> = matches.iter()
    ...
    for m in &matches {
        println!("  {} = '{}' (lines {}-{})", 
            m.capture_name, m.text, m.start_line + 1, m.end_line + 1);
    }
}
...
"#;
    ...
    let parsed = analyzer.parse(source, Metal::Rust).expect("Failed to parse");
    let matches = analyzer.run_query(&parsed, QueryType::Complexity).expect("Failed to run query");
    
    let branches = matches.iter()
