...
fn test_available_languages() {
    ...
    for metal in Metal::all() {
        ...
        println!("{:?}: {}", metal, if available { "✓ Available" } else { "✗ Not available" });
    }
    
    // At least Rust and Go should work
    assert!(analyzer.has_parser(Metal::Rust), "Rust parser should be available");
    assert!(analyzer.has_parser(Metal::Go), "Go parser should be available");
}